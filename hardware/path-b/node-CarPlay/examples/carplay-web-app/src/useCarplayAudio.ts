  ...
  useEffect(() => {
    const initMic = async () => {
      try {
        ...
        // This unit has no capture hardware at all (confirmed via `arecord -l`) -- surfacing
        // this on screen would just be a permanent, unactionable warning on every cold boot.
        // Console-only until a mic is actually wired up.
